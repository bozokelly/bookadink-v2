// game-reminder-24h — Supabase Edge Function
...
//       url := 'https://<project-ref>.supabase.co/functions/v1/game-reminder-24h',
...
//
// Deploy: supabase functions deploy game-reminder-24h --no-verify-jwt
...
serve(async (_req: Request) => {
  ...
  // Find games starting in the 1h45m–2h15m window from now (30-minute window to absorb cron jitter)
  const windowStart = new Date(Date.now() + 105 * 60 * 1000).toISOString();
  const windowEnd   = new Date(Date.now() + 135 * 60 * 1000).toISOString();

  // Fetch games in window, filter client-side to exclude cancelled
  // (PostgREST can't cleanly express "status IS NULL OR status != 'cancelled'")
  ...
  for (const game of activeGames) {
    ...
    // Deduplication: find users who already have a 24h reminder notification for this game
    const { data: alreadySent } = await supabase
      .from("notifications")
      .select("user_id")
      .eq("type", "game_reminder_24h")
    ...
    await Promise.allSettled(
      pendingBookings.map(async (booking: { id: string; user_id: string }) => {
        ...
        if (notifError) {
          ...
          return; // Don't push if row insert fails
        }
        ...
        try {
          await sendAPNS({
            ...
            title: pushTitle,
            body: pushBody,
            data: { game_id: game.id, type: "game_reminder_24h" },
            threadID: `game.${game.id}`,
            collapseID: `reminder.${game.id}`,  // collapse multiple reminder attempts
          });
          totalPushed++;
          await supabase.from("push_notification_log").insert({
            ...
            title: pushTitle,
            body: pushBody,
            payload: { game_id: game.id, type: "game_reminder_24h" },
            apns_status: 200,
          });
        } catch (err) {
