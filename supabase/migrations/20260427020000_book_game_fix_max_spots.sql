-- Fix book_game: two bugs corrected in one migration.
--
-- BUG 1 (42703): games column is max_spots, not max_players.
--   Both prior migrations referenced games.max_players which does not exist.
--
-- BUG 2 (42702): RETURNS TABLE(id UUID, ...) creates an implicit PL/pgSQL
--   output variable named `id`. PostgreSQL 14+ treats `bookings.id` in the
--   RETURNING clause as ambiguous between that variable and the table column.
--   Fix: pre-generate the booking UUID via gen_random_uuid() and INSERT with
--   explicit id — eliminates the RETURNING clause entirely.
--
-- This is the single canonical definition of book_game going forward.

DROP FUNCTION IF EXISTS book_game(UUID, UUID);
DROP FUNCTION IF EXISTS book_game(UUID, UUID, BOOLEAN, TEXT, TEXT, INT, INT, INT);
...
BEGIN
    ...
    -- Cast status::text because booking_status is a PG enum.
    SELECT COUNT(*) INTO v_confirmed_cnt
    FROM   bookings
    WHERE  bookings.game_id = p_game_id
      AND  bookings.status::text IN ('confirmed', 'pending_payment');

    IF v_max_spots IS NOT NULL AND v_confirmed_cnt >= v_max_spots THEN
        -- Game is full — place on waitlist regardless of hold flag.
        SELECT COALESCE(MAX(bookings.waitlist_position), 0) + 1 INTO v_waitlist_pos
        FROM   bookings
        WHERE  bookings.game_id = p_game_id
          AND  bookings.status::text = 'waitlisted';
    ...
    -- Pre-generated UUID in v_booking_id avoids the RETURNING clause, which
    -- would be ambiguous: RETURNS TABLE(id UUID) creates an output variable
    -- also named `id`, confusing PostgreSQL 14+ in the RETURNING context.
